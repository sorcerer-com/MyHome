...
def get_flac_converter():
    ...
    if flac_converter is None: # flac utility is not installed, check for current dir
        path = os.path.dirname(os.path.abspath(__file__))  # directory  the current module file, where all the FLAC bundled binaries are stored
        p = os.path.join(path, "flac")
        if os.path.exists(p) and os.access(p, os.X_OK):
            flac_converter = p
    if flac_converter is None:  # flac utility is not installed
        base_path = os.path.dirname(os.path.abspath(__file__))  # directory  the current module file, where all the FLAC bundled binaries are stored
