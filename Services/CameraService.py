...
class CameraService:
	@staticmethod
	def saveImage(fileName, size, frames, skipFrames):
		...
		try:
			subprocess.call(["fswebcam", "-r " + size, "-F " + str(frames), "--deinterlace", "-S " + str(skipFrames), fileName], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
			return True
