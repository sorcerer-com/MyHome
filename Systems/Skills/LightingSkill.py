...
class LightingSkill(BaseSkill):
    """ LightingSkill class """

    @type_check
    def __init__(self, owner: None) -> None:
        """ Initialize an instance of the LightingSkill class.
        ...
        if event == "SensorDataAdded" and "Motion" in data and len(data) == 1:
            ...
            if "Lighting" in latestData and latestData["Lighting"] < self.lightOnThreshold:
                ...
                for driver in lightDrivers:
                    logger.debug("Light %s: %s (%s)", driver.name,
                                 driver.isOn, driver in self._lightsOn)
                    ...
                    logger.debug("Light '%s' turn on until: %s",
                                 driver.name, self._lightsOn[driver])
