...
public class TuyaIRControl : TuyaDevice
{
    ...
    public async Task<string> GetButtonCodeAsync(int timeout, int retries = 2, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            await this.SetDpsAsync(new Dictionary<int, object>() { { 201, subCmdJson } }, cancellationToken: cancellationToken);

            while (true)
            {
                var response = await this.SetDpsAsync(new Dictionary<int, object>() { { 201, subCmdJson } }, overrideRecvTimeout: timeout, allowEmptyResponse: true, cancellationToken: cancellationToken);
                if (response != null)
                {
    ...
    public async Task SendButtonCodeAsync(string buttonCode, int retries = 2, int? overrideRecvTimeout = null, CancellationToken cancellationToken = default)
    {
        ...
        var subCmdJson = JsonConvert.SerializeObject(subCmd);
        await this.SetDpsAsync(new Dictionary<int, object>() { { 201, subCmdJson } }, retries: retries,
    ...
    public static ushort[] HexToPulses(string codeHex)
    {
        var pulses = Enumerable.Range(0, codeHex.Length)
            .Where(x => (x % 4) == 0)
            .Select(x => Convert.ToUInt16(codeHex.Substring(x + 2, 2) + codeHex.Substring(x, 2), 16));
